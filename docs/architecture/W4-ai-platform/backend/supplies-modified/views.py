...
# ── 向量相似度优化 ──────────────────────────────────────
# 优先用 numpy 算点积（1536 维向量 × 100 条 < 5ms）
# 没装 numpy 时降级到纯 Python（更慢但能用）
try:
    import numpy as np
    _HAS_NUMPY = True
except ImportError:
...
def _cosine_similarity(vec_a, vec_b):
    """计算两个向量的余弦相似度（numpy 加速 + 纯 Python 兜底）"""
    if _HAS_NUMPY and vec_a and vec_b:
...
def _batch_cosine_similarity(query_vec, candidate_vecs):
    """
    批量相似度：query vs 多个 candidate，一次性算完
    有 numpy 时是单个矩阵乘法，比逐个调用 _cosine_similarity 快 50-100 倍
    """
    ...
    if _HAS_NUMPY:
        try:
            ...
            Q = np.asarray(candidate_vecs, dtype=np.float32)
            # 余弦相似度 = Q·q / (|Q|·|q|)
            dots = Q @ q
            norms = np.linalg.norm(Q, axis=1) * np.linalg.norm(q)
            norms = np.where(norms == 0, 1.0, norms)  # 防 0 除
            sims = dots / norms
            return sims.tolist()
        except Exception:
            pass
    # fallback
    return [_cosine_similarity(query_vec, v) for v in candidate_vecs]
...
class SupplyViewSet(viewsets.GenericViewSet):
    ...
    def feed(self, request):
        ...
        # 走向量索引检索 top 候选（内部懒加载 + TTL 缓存）
        from ai.vector_index import search_supply_similar
        exclude_uuids = [str(profile.uuid)]  # 不推荐自己的
        top_results = search_supply_similar(
            user_emb, top_k=20, exclude_uuids=exclude_uuids,
        )

        if not top_results:
            return Response({'code': 0, 'data': {'items': []}})

        # 拿到候选 uuid 列表后，一次性查 DB 拿详细信息
        uuids = [u for u, _ in top_results]
        sim_map = dict(top_results)

        # 用 __in 一次查，避免 N+1
        supply_objs = Supply.objects.filter(uuid__in=uuids).select_related('profile__user')
        # 保持索引相似度顺序
        ordered = {str(s.uuid): s for s in supply_objs}
        scored = [(sim_map.get(str(s.uuid), 0), s) for s in [ordered[u] for u in uuids if u in ordered]]

        items = []
        for sim, supply in scored:
