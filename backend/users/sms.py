...
def send_sms(phone, code, template_code=None, sign_name=None):
    """
    发送短信验证码
    phone: 手机号
    code: 验证码
    template_code: 短信模板CODE，默认用环境变量或 settings 配置
    sign_name: 签名，默认用环境变量或 settings 配置
    返回: (success, message)
    """
    ...
    sign_name = sign_name or ALIYUN_SMS_SIGN_NAME
